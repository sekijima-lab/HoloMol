...
public class InputSourceHands : InputSourceBase, ITargetingInputSource
{
    ...
    private void Awake() {
#if UNITY_WSA
        UnityEngine.XR.WSA.Input.InteractionManager.InteractionSourceDetectedLegacy += WSASourceEntered;
        UnityEngine.XR.WSA.Input.InteractionManager.InteractionSourceLostLegacy += WSASourceLost;
        UnityEngine.XR.WSA.Input.InteractionManager.InteractionSourceUpdatedLegacy += WSASourceUpdate;

        UnityEngine.XR.WSA.Input.InteractionManager.InteractionSourcePressedLegacy += WSAFingerPressed;
        UnityEngine.XR.WSA.Input.InteractionManager.InteractionSourceReleasedLegacy += WSAFingerReleased;
#endif
    }
    ...
    public void WSASourceEntered(UnityEngine.XR.WSA.Input.InteractionSourceState state) {
        ...
        if (state.source.kind == UnityEngine.XR.WSA.Input.InteractionSourceKind.Hand) {
            ...
            state.properties.sourcePose.TryGetPosition(out inputState.Position);
    ...
    public void WSASourceUpdate(UnityEngine.XR.WSA.Input.InteractionSourceState state) {
        if (state.source.kind == UnityEngine.XR.WSA.Input.InteractionSourceKind.Hand) {
            Vector3 newPosition;
            if (state.properties.sourcePose.TryGetPosition(out newPosition)) {
    ...
    private void UpdateFromWSASource(CurrentHandState currentInputState, UnityEngine.XR.WSA.Input.InteractionSourceState state) {
        ...
        state.properties.sourcePose.TryGetVelocity(out currentInputState.Velocity);
